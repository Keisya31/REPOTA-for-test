<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-7">
        <div class="relative flex items-center justify-center mb-14">
            ...
            <h1 class="text-3xl font-extrabold tracking-tight text-gray-900">FORUM DISKUSI UTAMA</h1>
        </div>
        

        <div class="flex-1 p-6 space-y-3 overflow-y-auto bg-[#F2F1E1] rounded-t-lg">
            @if($threads ->isNotEmpty())
                ...
                <div class="flex items-start gap-4 ml-12 mt-2">
                    <div class="flex-shrink-0"></div> <div class="flex-1">
                        <div class="flex items-start gap-3">
                            ...
                            <span class="font-bold text-sm">{{ $t->nama_pengirim }}</span>
                            <span class="font-bold text-xs">{{ $t->created_at->format('d-m-Y H:i') }}</span>
                        </div>
                        
                        <div class="mt-1 bg-white border border-gray-400 p-2 ml-10 rounded-sm">
                            <p class="text-sm text-gray-700">{{ $t->isi_pesan }}</p>  
                        </div>
                        <a href="{{ route("forum.thread", $t->id) }}" class="text-sm text-gray-500 hover:text-gray-700 underline px-10 ">Lihat Diskusi</a>
                    </div>
                </div>
        
                <div id="display-{{ $t->id }}" class="items-start ml-28 ">
                    @php
                        $isOwner = false;
                        ...
                        elseif (!$t->user_id && $t->session_token === session()->getId()) {
                            $isOwner = true;
                        }
                    ...
                    <div class="flex gap-2 text-xs mt-2">
                        @if($isOwner)
                            <button onclick="toggleEdit({{ $t->id }})" class="text-blue-500 hover:underline">Edit</button>
                        @endif

                        @if($isOwner || $isAdmin)
                            <form action="{{ route('forum.destroy', $t->id) }}" 
                                method="POST" 
                                id="delete-form-{{ $t->id }}" 
                                class="inline">
                                @csrf @method('DELETE')
                ...
                <div id="form-{{ $t->id }}" class="hidden mt-2">
                    <form action="{{ route('forum.update', $t->id) }}" method="POST">
                        @csrf @method('PUT')
                        <textarea name="isi_pesan" class="w-full border-black border-2 p-2 text-sm">{{ $t->isi_pesan }}</textarea>
                        <div class="flex gap-2 mt-1">
                            <button type="submit" class="bg-black text-white px-3 py-1 text-xs">Simpan</button>
                            <button type="button" onclick="toggleEdit({{ $t->id }})" class="text-xs underline">Batal</button>
                        </div>
                    </form>
                </div>
                @endforeach
