    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="relative flex items-center justify-center mb-8">
            <button class="absolute left-0 text-sm font-medium text-gray-600 hover:text-black transition">
                <a href="{{ route("forum") }}" class="mr-1"> < Kembali</a>
            </button>
            <h1 class="text-3xl font-extrabold tracking-tight text-gray-900">FORUM DISKUSI TOPIK SKRIPSI</h1>
        </div>
        ...
        <div class="flex flex-col lg:flex-row gap-6 bg-[#F2F1E1] rounded-lg overflow-hidden shadow-sm min-h-[600px]">
            ...
            <div class="flex-1 flex flex-col">
                <div class="p-6 border-b border-gray-300">
                    <h3 class="text-xs uppercase tracking-widest text-gray-500 font-semibold">Diskusi Topik Skripsi</h3>
                    <h2 class="font-bold text-xl uppercase">{{ $skripsi->judul }}</h2>
                </div>
                ...
                <div class="flex-1 p-6 space-y-3 overflow-y-auto">
                        ...
                        <div class="flex items-start gap-4">
                            <div class="flex-shrink-0"></div> <div class="flex-1">
                                <div class="flex items-start gap-3">
                                    ...
                                    <span class="font-bold text-sm">{{ $threads->nama_pengirim }}</span>
                                    <span class="font-bold text-xs">{{ $threads->created_at->format('d-m-Y H:i') }}</span>
                                </div>
                                <div class="mt-1 bg-white border-2 border-black p-3 ml-10 relative shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)]">
                                    <p class="text-gray-800">{{ $threads->isi_pesan }}</p>
                                </div>
                            </div>
                        </div>
                    ...
                    @else
                        <div class="text-center py-10">
                            <p class="text-gray-500 italic">Belum ada diskusi. Jadilah yang pertama memulai!</p>
                        </div>
                    @endif
                </div>
                <script>
                ...
                <div class="p-4 bg-[#E8E7D5] border-t border-gray-300">
                    <form action="{{ $threads ? route('forum.reply') : route('forum.store') }}" method="POST" class="flex items-center gap-4">
                        @csrf
                        ...
                        @if($threads)
                            <input type="hidden" name="parent_id" value="{{ $threads->id }}">
                            <input type="hidden" name="skripsi_id" value="{{ $threads->skripsi_id }}">
                        @else
            ...
            <div class="w-full lg:w-80 bg-white p-6 border-l border-gray-300">
                <button class="w-full bg-[#FF5722] text-white font-bold py-2 px-4 rounded shadow-md mb-8 hover:bg-orange-600 transition">
                    <a href={{ route('forum') }}>All Rooms</a>
                </button>
                ...
                <div class="space-y-4">
                    ...
                    <a href={{ route('detail.skripsi', $skripsi->id) }} class="text-blue-500 hover:underline text-xs">Lihat detail skripsi</a>
                    <p class="text-xs text-gray-600 leading-relaxed">
                        Room ini berisi diskusi terkait topik {{ $skripsi->judul }}. Gunakan bahasa yang sopan dalam berdiskusi.
                    </p>
                </div>
            </div>
        </div>
